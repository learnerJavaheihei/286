			...
			public String handle(String[] args)
			{
				...
				if(args.length == 0 || args[0].isEmpty())
				{
					...
					try
					{
						...
						FileUtils.writeStringToFile(new File("stats/RunnableStats-" + new SimpleDateFormat("MMddHHmmss").format(System.currentTimeMillis()) + ".txt"), RunnableStatsManager.getInstance().getStats().toString(), "UTF-8");
		...
		_commands.add(new TelnetCommand("threads", "t"){
			...
			public String handle(String[] args)
			{
				...
				if(args.length == 0 || args[0].isEmpty())
				{
					...
					{
						new File("stats").mkdir();
						FileUtils.writeStringToFile(new File("stats/ThreadsDump-" + new SimpleDateFormat("MMddHHmmss").format(System.currentTimeMillis()) + ".txt"), StatsUtils.getThreadStats(true, true, true).toString(), "UTF-8");
