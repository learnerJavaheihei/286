...
public class CharacterSubclassDAO
{
	...
	public static final String SELECT_SQL_QUERY = "SELECT class_id, exp, sp, curHp, curCp, curMp, active, type, active_element_id FROM character_subclasses WHERE char_obj_id=?";
	public static final String INSERT_SQL_QUERY = "INSERT INTO character_subclasses (char_obj_id, class_id, exp, sp, curHp, curMp, curCp, maxHp, maxMp, maxCp, level, active, type, active_element_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean insert(int objId, int classId, long exp, long sp, double curHp, double curMp, double curCp, double maxHp, double maxMp, double maxCp, int level, boolean active, SubClassType type, ElementalElement activeElement)
	{
		Connection con = null;
	...
	public boolean store(Player player)
	{
		...
		try
		{
			...
			for(SubClass subClass : player.getSubClassList().values())
			{
				...
				sb.append("active_element_id=").append(subClass.getActiveElement().ordinal());
