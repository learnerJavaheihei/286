...
public final class WorldRegion implements Iterable<GameObject>
{
	...
	void addToObservers(GameObject object, Creature dropper)
	{
		...
		for(GameObject obj : this)
		{
			if(obj.getObjectId() == oid || obj.getReflectionId() != rid)
				continue;
			// Если object - игрок или наблюдатель, показать ему все видимые обьекты в регионе
			if(player != null)
			...
			{
				p = obj.getPlayer();
				/*if(p.isPlayer() && p.isInObserverMode())
					continue;*/
	...
	void removeFromObservers(GameObject object)
	{
		...
		for(GameObject obj : this)
		{
			...
			// Если object - игрок или наблюдатель, убрать у него все видимые обьекты в регионе
			if(player != null)
