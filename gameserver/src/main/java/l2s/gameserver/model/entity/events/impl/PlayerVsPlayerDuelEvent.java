...
public class PlayerVsPlayerDuelEvent extends DuelEvent
{
	private class DuelListeners implements OnAttackListener, OnMoveListener, OnTeleportedListener
	{
		...
		{
			if(actor.isInZoneBattle() || actor.isInPeaceZone() || actor.isInWater() || actor.isInSiegeZone() || actor.isInZone(Zone.ZoneType.no_restart)) // TODO: Проверить условия на оффе.
				stopEvent(false);
			else
			{
