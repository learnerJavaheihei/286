...
public class Olympiad
{
	...
	public static final int[][] BUFFS_LIST = { { 4357, 1 }, // Haste
			{ 4355, 1 }, // Acumen
			{ 4342, 1 }, // Wind Walk
			{ 4345, 1 }, // Might
			{ 4344, 1 }, // Shield
	...
	public static ScheduledFuture<?> _scheduledDailyTask;
	...
	public static void load()
	{
		...
		_dayStartTime = ServerVariables.getLong("olympiad_day_start_time", olympiadProperties.getProperty("day_start_time", 0L));
		...
		if(_period == 0)
		{
			_log.info("Olympiad System: Next Day Change is in....");

			milliToEnd = getMillisToDayChange();
	...
	public static void init()
	{
		...
		if(_scheduledDailyTask != null)
			_scheduledDailyTask.cancel(false);
		_scheduledDailyTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new DailyTask(), getMillisToDayChange(), Config.ALT_OLY_DPERIOD);
	}
	...
	{
		return Config.CLASSED_GAMES_ENABLED && getCompDay() == 4;// Классовые бои только по 4-м неделям
	}
	...
	public static synchronized boolean registerParticipant(Player player, CompType type)
	{
		...
		switch(type)
		{
			case CLASSED:
			{
				...
				{
					_classBasedRegisters.put(ClassId.valueOf(getParticipantClass(player.getObjectId())).getType2().ordinal(), player.getObjectId());
	...
	public static boolean validPlayer(Player sendPlayer, Player validPlayer, CompType type, boolean gameValidation)
	{
		...
		if(!gameValidation)
		{
			int[] ar = getDailyGameCounts(validPlayer.getObjectId());
	...
	public static void setDayStartTime(long value)
	{
		_dayStartTime = value;
	}
	...
	{
		return _dayStartTime + Config.ALT_OLY_DPERIOD;
	}

	private static long getMillisToDayChange()
	{
		return Math.max(10L, getDayEndTime() - System.currentTimeMillis());
	}

	public static synchronized void doDailyTasks()
	{
		if(isValidationPeriod())
			return;
	...
	public static synchronized int[] getDailyGameCounts(int objId)
	{
		int[] ar = new int[4];
