...
public class BookMarkList
{
	...
	private static final int[] TELEPORT_SCROLLS = { 13016 };
	...
	public boolean tryTeleport(int slot)
	{
		...
		if (Config.enablePremiumAccountPeaceZone && !MyUtilsFunction.isPremiumAccountPeaceZone(owner,owner.bookmarkLocation) && owner.getLevel() > 44){
			String msg = "等级大于45级的非会员角色不能传送到非安全区区域！";
...
//		}
	...
	public static boolean checkTeleportConditions(Player player,Location loc)
	{
		if(player == null)
			return false;
		...
		if (Config.enablePremiumAccountPeaceZone && !MyUtilsFunction.isPremiumAccountPeaceZone(player,loc) && player.getLevel() > 44){
			String msg = "等级大于45级的非会员角色不能传送到非安全区区域！";
