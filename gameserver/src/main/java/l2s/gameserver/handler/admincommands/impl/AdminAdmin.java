		...
		if(activeChar.getPlayerAccess().Menu)
		{
			...
			switch(command)
			{
				...
				case admin_showmovie:
					...
					break;
				case admin_showtutorial:
					if(wordList.length < 2)
					{
						Functions.sendDebugMessage(activeChar, "USAGE: //showtutorial id");
						return false;
					}
					int tutorialId;
					try
					{
						tutorialId = Integer.parseInt(wordList[1]);
					}
					catch(NumberFormatException e)
					{
						Functions.sendDebugMessage(activeChar, "You must specify id");
					...
					activeChar.sendPacket(new ExTutorialShowId(tutorialId));
					break;
