...
public class BotCheckManager
{
	...
	{
		ThreadPoolManager.getInstance().schedule(new BotQuestionAsked(), Rnd.get(Config.MINIMUM_TIME_QUESTION_ASK*60000, Config.MAXIMUM_TIME_QUESTION_ASK*60000)); //will make random to make them confused
	}
		...
		public void run()
		{
			for(Player player : GameObjectsStorage.getPlayers(false, false))
			{
				...
				for(Creature mob : World.getAroundNpc(player, 1000, 1000))
					if(mob.isMonster())
					{
						player.requestCheckBot(); //if the player doesn't appears to be to cond above asking the question and have a monster near by send question
						break;
					}	
