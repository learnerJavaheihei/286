...
import l2s.gameserver.network.l2.c2s.RequestExAutoplaySetting;
...
public class Cfg extends ScriptVoiceCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player activeChar, String args)
	{
				...
				if(param.length == 2)
				{
					...
					if(Config.AUTO_LOOT_INDIVIDUAL)
					{
						if(param[0].equalsIgnoreCase("autoloot")){
							...
							activeChar.sendPacket(new ExAutoplaySetting(activeChar));
						}
		...
		String disableMessage_result = "";
		String enableMessage_result = "";
		disableMessage_result = (activeChar.getLanguage().getShortName().equalsIgnoreCase("zh-tw") || activeChar.getLanguage().getShortName().equalsIgnoreCase("zh")) ? "禁用":disableMessage;
		enableMessage_result = (activeChar.getLanguage().getShortName().equalsIgnoreCase("zh-tw") || activeChar.getLanguage().getShortName().equalsIgnoreCase("zh")) ? "啟用":disableMessage;
		...
		if(Config.AUTO_LOOT_INDIVIDUAL)
		{
			...
			if(Config.AUTO_LOOT_ONLY_ADENA)
			{
				...
				if(activeChar.isAutoLootOnlyAdenaEnabled())
					autolootAdena = autolootAdena.replace("<?value_name_adena?>", disableMessage_result);
				else
					autolootAdena = autolootAdena.replace("<?value_name_adena?>", enableMessage_result);
			}
			...
			if(activeChar.isAutoLootEnabled())
				autolootBlock = autolootBlock.replace("<?value_name_items?>", disableMessage_result);
			else
				autolootBlock = autolootBlock.replace("<?value_name_items?>", enableMessage_result);
			...
			if(activeChar.isAutoLootHerbsEnabled())
				autolootBlock = autolootBlock.replace("<?value_name_herbs?>", disableMessage_result);
			else
				autolootBlock = autolootBlock.replace("<?value_name_herbs?>", enableMessage_result);
		}
		...
		if(activeChar.getVarBoolean("NoExp"))
		{
			...
			dialog = dialog.replace("<?value_name_noe?>", disableMessage_result);
		}
		else
		{
			dialog = dialog.replace("<?value_noe?>", "on");
			dialog = dialog.replace("<?value_name_noe?>", enableMessage_result);
		}

		if(activeChar.getVarBoolean("notraders"))
		{
			dialog = dialog.replace("<?value_notraders?>", "of");
			dialog = dialog.replace("<?value_name_notraders?>", disableMessage_result);
		}
		else
		{
			dialog = dialog.replace("<?value_notraders?>", "on");
			dialog = dialog.replace("<?value_name_notraders?>", enableMessage_result);
		}

		if(activeChar.getVarBoolean("notShowBuffAnim"))
		{
			dialog = dialog.replace("<?value_notShowBuffAnim?>", "of");
			dialog = dialog.replace("<?value_name_notShowBuffAnim?>", disableMessage_result);
		}
		else
		{
			dialog = dialog.replace("<?value_notShowBuffAnim?>", "on");
			dialog = dialog.replace("<?value_name_notShowBuffAnim?>", enableMessage_result);
		}
		...
		if(tl == null)
		{
			...
			dialog = dialog.replace("<?value_name_translit?>", enableMessage_result);
		}
