	...
	private boolean vote(String command, Player activeChar, String args)
	{
		...
		for(Entry<Integer, Vote> e : VoteList.entrySet())
			if(e.getValue().active)
			{
				...
				{
					List<Entry<Integer, String>> variants = new ArrayList<Entry<Integer, String>>(e.getValue().variants.entrySet());
					...
					loop: for(Entry<Integer, String> variant : variants)
					{
						...
						html.append("<a action=\"bypass -h user_vote " + e.getValue().id + " " + variant.getKey() + "\">" + variant.getValue() + "</a><br1>");
					}
					html.append("<br>");
				}
			}
